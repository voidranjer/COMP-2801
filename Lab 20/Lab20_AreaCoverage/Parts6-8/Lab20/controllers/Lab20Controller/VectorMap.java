    ...
    try {
      ...
      for (Obstacle ob: obstacles) {
        ...
        for (int i=0; i<vertices.size()-1; i++) {
            ...
            aPen.drawLine(vertices.get(i).x, 
                          height*magnification - vertices.get(i).y, 
                          vertices.get(i+1).x, 
                          height*magnification - vertices.get(i+1).y);
        }
        aPen.drawLine(vertices.get(0).x, 
                      height*magnification - vertices.get(0).y, 
                      vertices.get(vertices.size()-1).x, 
                      height*magnification - vertices.get(vertices.size()-1).y);
        
        // Now Draw the vertices
        for (int i=0; i<vertices.size(); i++) {
            aPen.setColor(VertexColor);
            aPen.fillOval(vertices.get(i).x-VERTEX_RADIUS, 
                          height*magnification - vertices.get(i).y-VERTEX_RADIUS, 
                          VERTEX_DIAMETER, VERTEX_DIAMETER);
            aPen.setColor(Color.black);
            aPen.drawOval(vertices.get(i).x-VERTEX_RADIUS, 
                          height*magnification - vertices.get(i).y-VERTEX_RADIUS, 
                          VERTEX_DIAMETER, VERTEX_DIAMETER);
        }
      }
