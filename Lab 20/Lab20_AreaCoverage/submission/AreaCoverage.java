// Author: Ben Cooper (SN: 101229286), James Yap(SN: 101276054), Breakout num: 26
...
public class AreaCoverage {
  ...
  public void computeGridGraph() {
    ...
    for (int r=0; r<numRows; r++) {
      for (int c=0; c<numCols; c++) {
        nodes[r][c] = new Node(new Point((int) (c*ROBOT_DIAMETER + ROBOT_RADIUS), (int) (r*ROBOT_DIAMETER + ROBOT_RADIUS)));
    ...
    for (Node n : gridGraph.getNodes()) {
      for (Obstacle obj : obstacles) {
        ...
        for (int i=0; i<obj.numVertices(); i++) {
          ...
          double distance = java.awt.geom.Line2D.ptSegDist(v1.x, v1.y, v2.x, v2.y, n.getLocation().x, n.getLocation().y);
  ...
  private void computeSpanningTreeFrom(Node n, Edge incomingEdge) {
    ...
    n.setVisited(true);
    incomingEdge.setSelected(true);

    for (Edge e : n.incidentEdges()) {
      Node otherNode = e.otherEndFrom(n);
  ...
  private void computeCoveragePathFrom(Node n, Edge e) {
    ...
    ArrayList<Node> neighbors = n.neighbours();
    Collections.sort(neighbors);

    for (Node neigh : neighbors) {
      Edge edge = n.getNeighbouringEdgeTo(neigh);
