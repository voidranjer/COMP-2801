...
public class MapperApp {
  ...
  public static boolean     ShowGridGraph    = true;
  public static boolean     ShowSpanningTree = true;
  public static boolean     ShowGridNodes    = true;
  ...
  public void drawSpanningTree() {
    ...
    for (Edge e: edges) {  // Draw the edges first
      if (e.isSelected()) {
        float d = e.length();
        int sx = (int)e.getStartNode().getLocation().x;
        int sy = (int)e.getStartNode().getLocation().y;
        int ex = (int)e.getEndNode().getLocation().x;
        int ey = (int)e.getEndNode().getLocation().y;
        int xDiff = ex - sx;
        int yDiff = ey - sy;
        for (int i=0; i<d; i++) {
