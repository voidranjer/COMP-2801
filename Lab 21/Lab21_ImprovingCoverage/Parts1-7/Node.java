...
public class Node implements Comparable<Node> {
  ...
  private java.awt.geom.Point2D.Double  location;        // location in the world
  ...
  // Some constructors
  public Node() { this("", new java.awt.geom.Point2D.Double(0,0)); }
  public Node(String aLabel) { this(aLabel, new java.awt.geom.Point2D.Double(0,0)); }
  public Node(java.awt.geom.Point2D.Double aPoint) { this("", aPoint); }
  public Node(Point aPoint) { this("", new java.awt.geom.Point2D.Double(aPoint.x, aPoint.y)); }
  public Node(String aLabel, Point aPoint) { this(aLabel, new java.awt.geom.Point2D.Double(aPoint.x, aPoint.y)); }
  public Node(java.awt.geom.Point2D.Double aPoint, Obstacle ob) { this("", aPoint); obstacle = ob; }
  public Node(Point aPoint, Obstacle ob) { this("", aPoint); obstacle = ob; }
  public Node(String aLabel, java.awt.geom.Point2D.Double aPoint) {
    label = aLabel;
    location = aPoint;
  ...
  public java.awt.geom.Point2D.Double getLocation() { return location; }
  ...
  public void setLocation(java.awt.geom.Point2D.Double aPoint) { location = aPoint; }
  public void setLocation(int x, int y) { location = new java.awt.geom.Point2D.Double(x, y); }
  ...
  public void draw(Graphics aPen, int height, int magnification) {
    ...
    aPen.fillOval((int)(MapperApp.MARGIN_X/2 +location.x*magnification - radius), 
                  (int)(MapperApp.MARGIN_Y/2 + height - location.y*magnification - radius), 
                  radius * 2, radius * 2);

    // Draw a black border around the circle
    aPen.setColor(Color.black);
    aPen.drawOval((int)(MapperApp.MARGIN_X/2 +location.x*magnification - radius), 
                  (int)(MapperApp.MARGIN_Y/2 + height - location.y*magnification - radius), 
                  radius * 2, radius * 2);

    // Draw a label at the top right corner of the node
    if (label != "")
      aPen.drawString(label, (int)(MapperApp.MARGIN_X/2 + location.x*magnification + radius), 
                      (int)(MapperApp.MARGIN_Y/2 + height - location.y*magnification - radius));
  }
