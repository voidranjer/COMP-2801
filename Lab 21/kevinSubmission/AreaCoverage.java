// Kevin Yuan 101261720 | James Yap 101276054
// Room 44
...
public class AreaCoverage {
  ...
  public ArrayList<Node> findBadNodes(ArrayList<Obstacle> obstacles, double width, double height) {
    ...
    for (Node startNode : selectedNodes) {
      Node previousNode = null;
      Node currentNode = startNode;
      Node nextNode = startNode.incidentEdges().get(0).otherEndFrom(startNode);
      boolean selectNextValidNode = true;
      boolean loopBroken = false;
  
      while (previousNode == null || !currentNode.getLocation().equals(startNode.getLocation())) {
        if (isInValid(currentNode, obstacles, width, height)) {
          badNodes.add(currentNode);
          selectNextValidNode = true;
          loopBroken = true;
        } else if (selectNextValidNode) {
          currentNode.setSelected(true);
          selectNextValidNode = false;
        }
        previousNode = currentNode;
        currentNode = nextNode;
        nextNode = nextNode.incidentEdges().get(0).otherEndFrom(nextNode);
  
        if (nextNode == previousNode && currentNode.incidentEdges().size() > 1) {
          nextNode = currentNode.incidentEdges().get(1).otherEndFrom(currentNode);
        }
      }
  
      if (loopBroken) {
        // Remove the copy of the start node
        badNodes.add(currentNode);
        // Connect the previous node to the original start node
        Edge newEdge = gridGraph.addEdge(previousNode, startNode);
        // Highlight the new edge
        newEdge.setSelected(true);
        // Unselect the start node
        startNode.setSelected(false);
      } else if (isInValid(currentNode, obstacles, width, height)) {
        badNodes.add(currentNode);
      }
    }
