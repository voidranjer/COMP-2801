  ...
  public void draw(Graphics aPen, int height, int magnification) {
    if (selected) {
      ...
      for (int i= -WIDTH/2; i<=WIDTH/2; i++) {
        if (yDiff > xDiff)
          aPen.drawLine(MapperApp.MARGIN_X/2 + (startNode.getLocation().x+i)*magnification, MapperApp.MARGIN_Y/2 + height - startNode.getLocation().y*magnification,
                        MapperApp.MARGIN_X/2 + (endNode.getLocation().x+i)*magnification, MapperApp.MARGIN_Y/2 + height - endNode.getLocation().y*magnification);
        else
          aPen.drawLine(MapperApp.MARGIN_X/2 +startNode.getLocation().x*magnification, MapperApp.MARGIN_Y/2 + height - (startNode.getLocation().y+i)*magnification,
                        MapperApp.MARGIN_X/2 +endNode.getLocation().x*magnification, MapperApp.MARGIN_Y/2 + height - (endNode.getLocation().y+i)*magnification);
      }
    }
    else {
