...
public class Lab12Controller {
  ...
  static void addSensorReadingsToMap(MapperApp mapper, Field translationField) {
    ...
    // double y = -(values[2] * 100) + 15; // due to StillMessyRoom
    ...
    // Apply each of the 9 IR sensor readings
    for (int i = 0; i < 9; i++) {
      ...
      if (d < 20) // This only records on the map if it was within the specified distance
        mapper.applySensorModelReading(sensorX, sensorY, (int) (a + aOffsets[i]), d, 1.72, 0.03);
    }

    // Apply each of the 5 sonar sensor readings
    for (int i = 9; i < 14; i += 1) {
      ...
      if (d < 20) // This only records on the map if it was within the specified distance
        mapper.applySensorModelReading(sensorX, sensorY, (int) (a + aOffsets[i]), d, 22.9, 0.066);
    }
    mapper.updateGrid(x, y);
  }
